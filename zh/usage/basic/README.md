...
- ### 数据结构
    ...
    \<2nd col\>：（数字或字符串）类作为X轴。
    
    \<3rd col\>：（字符串）作为颜色和图例的组。
...
- ### 参数详解
    ...
    **特殊参数**
    ...
    Errorbar Width: 误差线宽度
    
    Bar Border Color: 误差线边框颜色

    **其他说明**

    两列数据 P 值为与所有样本进行比较计算得到
...
- 简介
  ...
  对李克特量表带数据进行可视化描述性统计分析## [线性回归（误差线）](/basic/line-errorbar)
